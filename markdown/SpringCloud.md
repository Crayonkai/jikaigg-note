...
1. 资源互斥：共享资源只能被一个线程占用。  --互斥锁。
   * 
2. 占有且等待：线程当前至少占有一个资源并还想请求其他线程持有的资源就会造成等待。  --等待对方释放资源。
   * 设置统一管理对象，同时获取占用线程需要的资源
3. 不可抢占：资源只能由持有它的线程释放，其他线程不可强行占用资源。  --无法释放对方持有的资源。
   * 超时释放，利用lock的方法
4. 循环等待：线程T1等待线程T2占有的资源，线程T2等待线程T1占有的资源，就形成了循环等待。--两个线程相互等待。
   * 有序执行
...
1. 一次申请的数据量太大，去数据库查询了很大的list到内存。

2. 内存资源耗尽未释放，创建的线程资源不用了及时释放。引用池化的思想。

3. 本身服务器资源不足，通过虚拟机参数设置调整程序的堆内存大小 jmap -heap查看对内存信息

定位：

1. 系统已经挂掉了。通过dump文件进行分析
2. 系统运行中还未OOM，可以通过查看java内存诊断工具如java自带的java visaulVM或者阿里的arthas工具查看占用内存较大的类
3. 
